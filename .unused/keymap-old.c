...
#define BACKLIGHT_TIMEOUT user_config.bl_timeout
#define USER_CONFIG_TIMEOUT user_config.bl_timeout

// Timeout HIDRAW COMMANDS
...
// #define PIBRICK_CMD_SET_TIMEOUT         0x01
// #define PIBRICK_CMD_GET_TIMEOUT         0x02
// #define PIBRICK_STATUS_OK               0x00
// #define PIBRICK_STATUS_INVALID_COMMAND  0x01

static uint32_t rgb_off_timer = 0;
static bool rgb_timeout_active = false;
static uint32_t rgb_timeout_duration = 0;
void panelRgbUpdate(uint8_t r, uint8_t g, uint8_t b);
void setPwmRgb(uint8_t r, uint8_t g, uint8_t b);
void setPwmRGBReset(void);
void housekeeping_task_user(void) {
    if (rgb_timeout_active &&
        timer_elapsed32(rgb_off_timer) >= rgb_timeout_duration) {
...
static uint16_t halfmin_counter = 0;
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [2] = LAYOUT(
        ...
        QK_BOOTLOADER /*KC_COPY*/,  KC_PASTE,    KC_PRINT_SCREEN,    KC_INS,    QK_MOUSE_BUTTON_3, KC_NO
    ),
    [3] = LAYOUT(
...
void setPwmRgbRaw(uint16_t r, uint16_t g, uint16_t b){
  pwmEnableChannel(&PWMD5, 0, 62500-r);
  pwmEnableChannel(&PWMD5, 1, g);
  pwmEnableChannel(&PWMD6, 0, 62500-b);
}
void panelRgbUpdate(uint8_t r, uint8_t g, uint8_t b) {
    const uint16_t PWM_MAX = 62500;
    uint16_t rpwm = ((uint32_t)r * PWM_MAX) / 255;
    uint16_t gpwm = (((uint32_t)g * PWM_MAX) / 255) * 0.4;
    uint16_t bpwm = (((uint32_t)b * PWM_MAX) / 255) * 0.7;
    setPwmRgbRaw(rpwm, gpwm, bpwm);
}
void setPwmRgbValue(uint16_t lvalue){
  ...
  setPwmRgbRaw(
    savedLED_r?lvalue:0,
    savedLED_g?((uint16_t) (lvalue*0.4)):0,
    savedLED_b?((uint16_t) (lvalue*0.7)):0
  );
...
void matrix_scan_user(void) {
  ...
  if (led_on && halfmin_counter >= user_config.bl_timeout * 2) {
    ...
    switch (data[1]) {
        ...
        case PIBRICK_CMD_RGB:
            ...
            if (r == 0 && g == 0 && b == 0) {
                ...
                panelRgbUpdate(r, g, b);

                if (duration > 0) {
                    rgb_timeout_duration = duration;
                    rgb_off_timer = timer_read32();
